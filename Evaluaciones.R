...
gruporespuestas_solo$interpretacion <-
  ordered(gruporespuestas_solotally$interpretacion,
          c("FIGURADO", "LITERAL", "DISTRACTOR"))
...
gruporespuestas_solotallyd <- gruporespuestas_solotally %>%
  ungroup() %>%
  mutate(interpretacion = case_when(interpretacion == "FIGURADO" ~ "FIGURADO",
                                    TRUE ~ "NO FIGURADO")) %>%
  select(-nn) %>%
  group_by(tratamiento, caracterizacion, interpretacion) %>%
  summarize(sum = sum(n)) %>%
...
grupogradointerpretacion_1$interpretacion <-grupogradointerpretacion_1$interpretacion %>% 
  ordered(c("FIGURADO","LITERAL", "DISTRACTOR"))
...
grupocontextotally <-
  grupocontexto %>% group_by(tratamiento, contexto) %>% add_tally()
...
grupocontextorespuestas_2tally <-
  grupocontextorespuestas_2 %>% group_by(tratamiento, contexto, pregunta) %>% add_tally()

ggplot(grupocontextorespuestas_2tally,
       aes(x = pregunta, y = n, fill = interpretacion)) +
  geom_bar(stat = "identity", position = "fill") +
  geom_text(aes(label = percent(signif(n / nn, 2))), position = position_fill(vjust = .5)) +
  coord_flip() +
